import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

import '../../../main.dart';
import '../theme/colors/colors_manager.dart';
import '../theme/texts/font_weight_helper.dart';

class TextStyles {
  static TextStyle font32DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 32.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font32DarkBlueSemiBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 32.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font24DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 24.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font24DarkBlueSemiBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 24.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font24RedSemiBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 24.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font20WhiteBold = TextStyle(
    color: Colors.white,
    fontSize: 20.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font20DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 20.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font16DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 16.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font16DarkBlueSemiBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 16.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font16WhiteSemiBold = TextStyle(
    color: Colors.white,
    fontSize: 16.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font16BlueGreyRegular = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 16.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14DarkBlueSemiBold() {
    return TextStyle(
      color: themeIsDark ? Colors.white : ColorsManager.darkBlue,
      fontSize: 14.sp,
      fontWeight: FontWeightHelper.semiBold,
      fontFamily: 'Montserrat',
    );
  }

  static TextStyle font14WhiteSemiBold = TextStyle(
    color: Colors.white,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14RedSemiBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14DarkBlueMedium = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14DarkBlueRegular = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14BlueGreyRegular = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font14WhiteLite = TextStyle(
    color: ColorsManager.grey,
    fontSize: 14.sp,
    fontWeight: FontWeightHelper.light,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12BlueGreyBold = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12RedBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12DarkBlueSemiBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12BlueGreySemiBold = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12WhiteSemiBold = TextStyle(
    color: Colors.white,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12RedSemiBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12DarkBlueMedium = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12BlueGreyMedium = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12DarkBlueRegular = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12BlueGreyRegular = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font12BlueGreyLight = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 12.sp,
    fontWeight: FontWeightHelper.light,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10DarkBlueBold = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10RedBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10BlueGreySemiBold = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10RedSemiBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10DarkBlueMedium = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10WhiteMedium = TextStyle(
    color: Colors.white,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10BlueGreyMedium = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10DarkBlueRegular = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10BlueGreyRegular = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10RedRegular = TextStyle(
    color: ColorsManager.red,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font10BlueGreyLight = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 10.sp,
    fontWeight: FontWeightHelper.light,
    fontFamily: 'Montserrat',
  );

  static TextStyle font8BlueGreyBold = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 8.sp,
    fontWeight: FontWeightHelper.bold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font8DarkBlueMedium = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 8.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font8BlueGreyMedium = TextStyle(
    color: ColorsManager.blueGrey,
    fontSize: 8.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );

  static TextStyle font8WhiteRegular = TextStyle(
    color: Colors.white,
    fontSize: 8.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font8DarkBlueRegular = TextStyle(
    color: ColorsManager.darkBlue,
    fontSize: 8.sp,
    fontWeight: FontWeightHelper.regular,
    fontFamily: 'Montserrat',
  );

  static TextStyle font7RedSemiBold = TextStyle(
    color: ColorsManager.red,
    fontSize: 7.sp,
    fontWeight: FontWeightHelper.semiBold,
    fontFamily: 'Montserrat',
  );

  static TextStyle font7WhiteMedium = TextStyle(
    color: Colors.white,
    fontSize: 7.sp,
    fontWeight: FontWeightHelper.medium,
    fontFamily: 'Montserrat',
  );
}
